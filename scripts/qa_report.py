...
def _render_word_com(docx:Path,pdf_path:Path)->tuple[bool,str]:
 ...
 result=_run(["powershell","-NoProfile","-ExecutionPolicy","Bypass","-Command",script])
...
def render_report(docx:Path,job_id:str)->dict[str,Any]:
 ...
 for name,function in (("external",lambda:_render_external(docx,render_dir,pdf_path)),("libreoffice",lambda:_render_soffice(docx,render_dir,pdf_path)),("word-com",lambda:_render_word_com(docx,pdf_path))):
 ...
 if not backend:
  return {"status":"unavailable","backend":None,"attempts":attempts,"pdf":None,"pages":[],"preview":None}
...
def check_report(source:Path,generated:Path,metadata_path:Path,require_render:bool=False)->tuple[dict[str,Any],Path]:
 ...
 checks={"docx_opens":True,"source_media_preserved":_media_count(source)==_media_count(generated),"all_additions_labeled":labels>=expected,"no_unlocated_marker":"未定位内容" not in text,"paragraph_length_ok":all(len(p.text)<=700 for p in document.paragraphs)}
 render=render_report(generated,str(metadata["job_id"])); structural_ok=all(checks.values()); visual_ok=render["status"]=="passed"; shipping_ready=structural_ok and (visual_ok or not require_render)
