...
def run_pipeline(source:Path,plan:Path,output_dir:Path,port:int=8765,open_browser:bool=True,launch_dashboard_server:bool=True,require_render:bool=False)->dict[str,Any]:
 ...
 metadata=json.loads(metadata_path.read_text(encoding="utf-8")); metadata["quality"]=quality; digest=hashlib.sha256(generated.read_bytes()).hexdigest(); metadata["generated_sha256"]=digest
 for item in metadata.get("generated_files",[]):
  if item.get("kind")=="annotated": item["sha256"]=digest
 render=quality.get("render",{}); metadata_path.write_text(json.dumps(metadata,ensure_ascii=False,indent=2),encoding="utf-8")
 url=launch_dashboard(output_dir,port,open_browser) if launch_dashboard_server else None
 return {"generated_report":str(generated),"generated_files":metadata.get("generated_files",[]),"metadata":str(metadata_path),"quality_data":str(quality_path),"render_status":render.get("status","unknown"),"dashboard_url":url,"local_only":True}
