...
description: >-
  Review and generate downloadable university experiment reports locally. Activates for blank templates, completed lab reports, 实验执行报告、修改报告、空白实验报告、已完成报告、完成度、正确性、截图、生成 DOCX、下载报告、前端页面、保留原字体、彩色修改. Codex classifies the uploaded document, reviews text and screenshots, writes an execution or revision plan, preserves original DOCX styles, inserts clearly labeled content with distinct fonts and colors, opens a loopback dashboard, and provides downloads. Uses no external model API, remote OCR, upload service, or cloud document processor. Never fabricates experiments, results, evidence, screenshots, citations, or verification.
license: MIT
