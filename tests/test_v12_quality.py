...
class V12QualityTests(unittest.TestCase):
 ...
 def test_structured_blocks_companion_qa_and_downloads(self)->None:
  with tempfile.TemporaryDirectory() as temp:
   ...
   self.assertIn("☐ 确认环境",text); self.assertNotIn("未定位内容",text); self.assertEqual(len(generated.tables),1)
   metadata=json.loads(metadata_path.read_text(encoding="utf-8")); self.assertEqual(len(metadata["generated_files"]),2); self.assertEqual(metadata["priority_counts"]["high"],1)
   quality,quality_path=check_report(source,output,metadata_path); self.assertTrue(quality["ok"])
   metadata=json.loads(metadata_path.read_text(encoding="utf-8")); metadata["quality"]=quality; metadata["generated_files"].append({"kind":"quality","label":"质量报告","name":quality_path.name}); metadata_path.write_text(json.dumps(metadata,ensure_ascii=False),encoding="utf-8")
   client=create_app(root/"out").test_client(); listing=client.get("/api/reports").get_json()["reports"][0]; self.assertEqual(len(listing["files"]),3)
   download=client.get(listing["files"][1]["download_url"]); self.assertEqual(download.status_code,200); download.close()

 def test_render_unavailable_is_explicit(self)->None:
  from unittest.mock import patch
  with tempfile.TemporaryDirectory() as temp:
   doc=Path(temp)/"report.docx"; Document().save(doc)
   with patch("qa_report._render_external",return_value=(False,"missing")), patch("qa_report._render_soffice",return_value=(False,"missing")), patch("qa_report._render_word_com",return_value=(False,"missing")):
